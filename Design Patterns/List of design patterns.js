...
Factory Method: Creates an instance of several derived classes, allowing the client code to create 
...
Adapter: Allows objects with incompatible interfaces to work together by providing a wrapper 
...
Facade: Provides a simplified interface to a set of interfaces in a subsystem, making it 
easier to use.
...
Model-View-Controller (MVC): Separates an application into three interconnected components - Model 
(data and logic), View (presentation), and Controller (user input handling).

Model-View-ViewModel (MVVM): Similar to MVC, but with emphasis on separating the UI logic from the 
UI presentation through the ViewModel.

Layered Architecture: Separates an application into different layers (e.g., presentation, business 
